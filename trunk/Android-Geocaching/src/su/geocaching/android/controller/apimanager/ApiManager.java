...
public class ApiManager implements IApiManager {
    ...
    /* (non-Javadoc)
    * @see su.geocaching.android.controller.apimanager.IApiManager#getGeoCashList(double, double, double, double)
    */
    ...
    private URL generateUrl(double maxLatitude, double minLatitude, double maxLongitude, double minLongitude) throws MalformedURLException {
        String request = URL + "?lngmax=" + (maxLongitude) + "&lngmin=" + (minLongitude) + "&latmax=" + (maxLatitude) + "&latmin=" + (minLatitude) + "&id="
                + ((int) (Math.random() * 1E6)) + "&geocaching=" + "d6970489a98b83cb7382f7db94d574df";
        return new URL(request);
    }

    /* (non-Javadoc)
     * @see su.geocaching.android.controller.apimanager.IApiManager#getGeoCacheByID(int)
     */
