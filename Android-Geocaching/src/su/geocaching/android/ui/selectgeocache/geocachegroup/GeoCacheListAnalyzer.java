...
public class GeoCacheListAnalyzer {
    ...
    private GeoCacheListAnalyzer(MapView map, List<GeoCache> geoCacheList) {
        ...
        HashMap<Pair, Pair[]> clusterPointMap = new KMeans((Pair[]) cacheCoordinatesMap.keySet().toArray(), centroids).getClusterMap();
        fillOverlayItemList(clusterPointMap);
    }

    private void fillOverlayItemList(HashMap<Pair, Pair[]> clusterPointMap) {
    ...
    private List<GeoCache> convertPairArrayToGeoCacheList(Pair[] cacheCoordinates) {
